...
### **Cập nhật & Cải tiến nổi bật**

#### **Tối ưu hóa Giao diện & Trải nghiệm Người dùng (UX Clean & Fast)**
- **Giới hạn Mô hình AI chuẩn hóa 8 Models**: Giới hạn chính xác 8 mô hình AI được Groq Cloud hỗ trợ chính thức (`llama-3.3-70b-versatile`, `groq/compound`, `groq/compound-mini`, `deepseek-r1-distill-llama-70b`, `openai/gpt-oss-120b`, `openai/gpt-oss-20b`, `qwen/qwen3.6-27b`, `llama-3.1-8b-instant`).
- **Xóa bỏ hoàn toàn Emoji trong toàn bộ Mã nguồn**: Loại bỏ 100% các ký tự emoji rối mắt, thay thế bằng biểu tượng SVG tinh tế, giúp giao diện chuẩn phẳng, hiện đại và cao cấp.
- **Nút Bấm Pill Link Hướng Dẫn Nhanh (`.xt-guide-pill-btn`)**: Loại bỏ các khung video nhúng cồng kềnh trong thẻ cấu hình. Thay bằng nút bấm nhỏ gọn dạng Pill Chip dẫn thẳng sang tab Video Hướng dẫn với hiệu ứng cuộn mượt và highlight tập trung.
- **Cấu hình Giọng đọc Gốc (Original TTS Sync)**: Đồng bộ đầy đủ 10 bộ phát âm TTS và danh sách giọng đọc cho văn bản gốc.
...
- **Theme Icon Động**: Tự động chuyển đổi biểu tượng **Mặt Trời  / Mặt Trăng ** mượt mà trên nút đổi giao diện Sáng/Tối.
